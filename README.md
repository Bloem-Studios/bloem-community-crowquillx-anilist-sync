...
4. Maps TMDB, TVDB, or IMDb season/episode numbering to AniList with the daily [AniBridge mappings](https://github.com/anibridge/anibridge-mappings) dataset.
5. Reads the existing AniList list entry and only advances progress. It never lowers progress and marks an entry complete when AniList's episode count is reached.
...
The Silo plugin contract currently supplies event-consumer configuration globally rather than per profile. Use one plugin installation per AniList account and set its profile ID.
